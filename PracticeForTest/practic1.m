%A study is conducted to compare heat loss in glass pipes, versus steel pipes of
%the same size. Various liquids at identical starting temperatures are run
%through segments of each type and the heat loss (in C*) is measured.
...
if H == 0
    n = n1 + n2 - 2;
    t1 = tinv(1 - alpha, n);
    [H1, P1, CI1, STATS1] = ttest2(X1, X2, 'Vartype', 'equal', 'Alpha', alpha);
    if H1 == 0
        fprintf("\nThe null hypothesis is not rejected!\n");
    ...
    n = 1/n;
    t1 = tinv(1-alpha, n);
    [H1, P1, CI1, STATS1] = ttest2(X1, X2, 'Vartype', 'unequal', 'Alpha', alpha);
    if H1 == 0
        fprintf("\nThe null hypothesis is not rejected!\n");
        fprintf("On average, steel and glass lose the same heat!\n");
    else
